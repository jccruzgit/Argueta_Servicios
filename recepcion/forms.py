...
from recepcion.models import Persona, Empresa , TipoPrueba, Agenda, TipoResultado, Evaluador, Ficha, Profile
from input_mask.widgets import InputMask, __all__
...
Constants
"""
ERROR_MESSAGE_USER = {'required':'El usuario es obligatorio', 'unique' : 'El usuario ya se encuentra registrado', 'invalid':'El username no es correo'}
...
class CreateEmpresaForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'nit': forms.TextInput(attrs={'class':'input-field', 'id':'nit', 'pattern':'^[0-9]{4}-[0-9]{6}-[0-9]{3}-[0-9]{1}$', 'placeholder':'xxxx-xxxxxx-xxx-x', 'data-inputmask':"'mask': '9999-999999-999-9'"}),
            'nrc': forms.TextInput(attrs={'class': 'input-field', 'id': 'nrc', 'pattern': '^[0-9]{6}-[0-9]{1}$', 'placeholder': 'xxxxxx-x', 'data-inputmask': "'mask': '999999-9'"}),
        }
...
class CrearAgendaForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'fechaEvaluar': forms.DateInput(format='%d-%m-%Y', attrs={'class': 'form-control', 'id':'fecha', 'type':'date'}),
